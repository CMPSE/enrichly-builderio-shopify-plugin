...
plugin_tools_1.registerCommercePlugin({
    ...
    const service = {
        product: {
            ...
            async search(search) {
                let results = await graphQLClient_1.executeGraphQLQuery(settings.get('storeDomain'), settings.get('storefrontAccessToken'), settings.get('apiVersion') || '2020-07', graphQLQueries_1.PRODUCT_SEARCH_QUERY, {
                    query: search,
                    first: 250,
                    productFilters: [
                        {
